...
To install Docker you can follow the [Get Started with Docker](https://docs.docker.com/mac/) guide.
...
2. Copy database configured file to use with docker and setup database:
    ...
    % docker-compose run web-dev rake db:create
    % docker-compose run web-dev rake db:migrate
    % docker-compose run web-dev rake db:seed

3. Assets:

  Before starting the app we need to install the assets:

    % docker-compose run web-dev bower install --allow-root
